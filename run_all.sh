#!/usr/bin/env bash
docker run -d --net=host --name=consul -v $HOME/jalgoarena/consul:/consul/config -e CONSUL_UI_BETA=true consul agent -server -ui -bootstrap-expect=1 -bind=192.168.63.21
echo "Consul started"
...
docker run -d --net=host --name=traefik -v $HOME/jalgoarena/traefik.toml:/etc/traefik/traefik.toml traefik
echo "API Started"
